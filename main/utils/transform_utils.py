...
def scale(x: torch.Tensor) -> torch.Tensor:
    ...
    B = x.shape[0]
    x_flat = x.view(B, -1)                             
    lo = torch.quantile(x_flat, 0.01, dim=1, keepdim=True)   
    hi = torch.quantile(x_flat, 0.99, dim=1, keepdim=True)   
    lo = lo.view(B, 1, 1)
    hi = hi.view(B, 1, 1)
    x = x.clamp(lo, hi)
    x_flat = x.view(B, -1)
    mn = x_flat.min(dim=1, keepdim=True).values.view(B, 1, 1)
    mx = x_flat.max(dim=1, keepdim=True).values.view(B, 1, 1)
    x = (x - mn) / (mx - mn).clamp_min(1e-8)
